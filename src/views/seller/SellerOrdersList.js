...
function SellerOrdersList() {
  ...
  const handleExport = () => {
    axios({
      ...
      params: { queryType: "Seller_Orders", limit: "online" },
      headers: {
  ...
  useEffect(() => {
    console.log(
      "read current routename:",
      location.pathname,
      location.pathname.slice(20),
      location.pathname.slice(20) === "all"
    );
    ...
    axios(config)
      .then((response) => {
        ...
        setResOrderList(
          response.data.data.filter((order) => order.order_type === "online")
        );

        if (location.pathname.slice(20) === "all") {
          setOrderList(
            response.data.data.filter((order) => order.order_type === "online")
          );
        } else {
          setOrderList(
            response.data.data
              ...
              .filter(
                (order) =>
                  order?.OrderDetails?.[0]?.order_status ===
                  location.pathname.slice(20)
              )
          );
        }
  ...
  useEffect(() => {
    if (location.pathname.slice(20) === "all") {
      setOrderList(
        resOrderList.filter((order) => order.order_type === "online")
      );
    } else {
      setOrderList(
        resOrderList
                      ...
                      {!!orderList.length &&
                        orderList.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td>{ISOToIST(item.order_date_time)}</td>
