...
function SellerRefundRequestList() {
  ...
  const getRefundRequestListAPI = () => {
    const config = {
      ...
      url:
        ...
        "/api/productReturn/cancelandreturnorder",
      headers: {
    ...
    axios(config)
      .then((response) => {
        ...
        // setCanceledOrderList(response.data.data.cancel_product);
        // if (location.pathname.includes("/pending")) {
        setCanceledOrderList(
          response.data.data.cancel_product.filter(
            (order) =>
              order.payment_refund_status === "refunded"
          )
        );
        setReturnedOrderList(
          response.data.data.return_product.filter(
            (order) =>
              order.payment_refund_status === "refunded"
          )
        );
  ...
  console.log(canceledOrderList);
  console.log(returnedOrderList);
  ...
  const updateRefundStatus = (productreturnId, payment_refund_status) => {
    const config = {
      ...
      url:
        process.env.REACT_APP_BASEURL +
        `/api/productReturn/paymentrefundstatus?_id=${productreturnId}`,
      // params: { productreturn_id: productreturnId },
      headers: {
        "Content-Type": "application/json",
        // "Content-Type": "multipart/form-data",
          ...
          <div className="flex-between align-items-center">
            <div>
              <h1 className="page-header-title">
                {/* IMP */}
                Refunded Refund Request List
                <span className="badge badge-soft-dark mx-2">
                  {canceledOrderList?.length + returnedOrderList?.length || 0}
                </span>
              </h1>
            </div>
            <div>
                ...
                {returnedOrderList &&
                  returnedOrderList.map((item) => (
                    <tr>
                      ...
                      <td>return</td>
                      <td>
                        {item.orderdetail?.[0]?.ordermaster?.[0]?.paymentmode}
                      </td>
                      <td>
                        {/* {item?.payment_refund_status || "pending"} */}
                        <select
