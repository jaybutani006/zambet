      ...
      if (e.target.files && !!e.target.files.length) {
        if (e.target.files.length <= 8) {
          ...
          if (!e?.target?.multiple) {
            setProductDetails((prev) => ({
              ...prev,
              [name]: [...prev?.[name], ...e.target.files],
              imageURLObj: [...prev?.imageURLObj, ...imageURLObj],
            }));
          } else {
            setProductDetails((prev) => ({
              ...prev,
                  ...
                  <div className="row">
                    {!!productDetails?._pphoto?.length &&
                      productDetails?._pphoto?.map((item, index) => (
                        <div
                          className="col-md-4 col-lg-2 justify-content-center align-items-center"
                          style={
                            {
