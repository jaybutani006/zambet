...
function AdminCategoryView() {
  ...
  console.log(mainState.allCategories);
  ...
  const handleDeleteCategory = (e, index, catId) => {
    if (
      window.confirm(
          `Do you Want to delete service-${index + 1}?`
      )
    ) {
      ...
      axios({
        method: "delete",
        url:
          process.env.REACT_APP_BASEURL + "/api/services?service_id=" + catId,
        headers: {
          Authorization: state.adminToken,
        },
      })
                          ...
                          >
                            ...
                            <td>
                              <Link
                                ...
                                to={"/admin/service/edit/" + item._id}
                                state={item}
                              >
