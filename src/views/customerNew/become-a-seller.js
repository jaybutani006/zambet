...
function BecomeASeller() {
  ...
  const [isDisabledApplyShop, setIsDisabledApplyShop] = useState(false);
  ...
  const validateFields = (e) => {
    ...
    if (!userDetails.f_name)
      return alert(`First Name can't be empty or invalid`);
    else if (userDetails.f_name.length < 3)
      return alert(`First Name must be greater than 3 characters`);

    if (!userDetails.l_name)
      return alert(`Last Name can't be empty or invalid`);
    else if (userDetails.l_name.length < 3)
      return alert(`Last Name must be greater than 3 characters`);

    if (!userDetails.email)
      return alert(`Email Address can't be empty or invalid`);
    else if (!emailRegex.test(userDetails.email)) {
      return alert("Invalid Email Address");
    }

    if (!userDetails.phone)
      return alert(`Phone Number can't be empty or invalid`);
    if (userDetails.phone.toString().length !== 10)
      return alert(`Phone Number must be of 10 digits only`);
    ...
    if (!userDetails.repeat_password)
      return alert(`Confirm Password can't be empty or invalid`);
    else if (userDetails.repeat_password.length < 8)
      return alert(`Confirm Password must be greater than 8 characters`);

    if (userDetails.repeat_password !== userDetails.password)
      return alert(`Confirm Password does not match with Password`);

    if (!userDetails.shop_name)
      return alert(`Shop Name can't be empty or invalid`);
    else if (userDetails.shop_name.length < 3)
      return alert(`Shop Name must be greater than 3 characters`);

    if (!userDetails.shop_address)
      return alert(`Shop Address can't be empty or invalid`);
    else if (userDetails.shop_address.length < 3)
      return alert(`Shop Address must be greater than 3 characters`);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "image") {
      ...
      if (e.target.value.toString().length > 10) {
        return alert("Phone number must be less than 10 digits");
      }
      ...
      if (!e.target.value) {
        ...
        setUserDetails((prev) => ({
          ...prev,
          [name]: e.target.value,
          [`${name}passwordStrength`]: passwordStrength(e.target.value).value,
        }));
      }
    } else {
      setUserDetails((previous) => {
  ...
  const handleApplyShop = () => {
    ...
    const data = JSON.stringify({
      ...
      contect_no: parseInt(userDetails.phone),
    ...
    console.log(data);
    // return;
    if (userDetails.vendor_type === "serviceProvider") {
      const config = {
        method: "post",
        ...
        data: {
          ...
          shop_address: userDetails.shop_address,
        },
      };
      axios(config)
        .then((response) => {
          ...
          window.confirm(
            `You have registered successfully for service provider.\nPlease wait for approval from admin.`
          );
  ...
  return (
    <>
      <Helmet>
        <script src="https://6valley.6amtech.com/public/assets/front-end/vendor/bs-custom-file-input/dist/bs-custom-file-input.min.js"></script>
        <script>{`
              ...
              $("#exampleInputPassword ,#exampleRepeatPassword").on(
                "keyup",
                function () {
      ...
      <div className="container main-card rtl" style={{ textAlign: "left" }}>
              ...
              <div className="col-lg-12">
                <div className="p-5">
                  ...
                  <form className="user">
                      ...
                      <div className="col-sm-6 mb-3 mb-sm-0">
                        ...
                        <div>
                          <small
                            style={{
                              color:
                                userDetails?.passwordpasswordStrength ===
                                "Strong"
                                  ? "green"
                                  : userDetails?.passwordpasswordStrength ===
                                    "Medium"
                        ...
                        <div className="form-group mb-1">
                          <strong>
                            <input
                              ...
                              name="remember"
                              id="inputCheckd"
                              checked={!isDisabledApplyShop}
                              onChange={(e) =>
                                setIsDisabledApplyShop(!e.target.checked)
                              }
                            />
                          </strong>
                          <label className htmlFor="remember">
                            ...
                            <a
                              ...
                              target="_blank"
                              href="/seller/termsAndConditions"
                            >
                    ...
                    <button
                      // type submit calling form's function
                      // type="submit"
                      disabled={isDisabledApplyShop}
                      className={
                        isDisabledApplyShop
                          ? "btn btn-dark btn-user btn-block"
                          : "btn btn-primary btn-user btn-block"
                      }
                      // removing id as vendor.min.js is also calling another api
                      // id="apply"
                      // disabled of jquery and reactjs , both have their own and thats making issues
                      // disabled
                      // data-toggle="modal"
                      // data-target="#verify-otp"
                      // onClick={(e) => handleApplyShop(e)}
 ...
 */}
                  ...
                  <div className="text-center">
                    <a className="small" href="/seller/auth/login">
