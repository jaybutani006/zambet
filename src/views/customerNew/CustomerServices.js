...
function CustomerProducts() {
  ...
  const doesSellerHaveSelectedServices = (seller) => {
    if (selectedServices.length === 0) {
      // No service selected, show all service providers
      return true;
    }

    for (const portfolio of seller["Service Provider Portfolios"]) {
      if (selectedServices.includes(portfolio.service_id)) {
        return true;
      }
    }
  ...
  const apiGetAllSellers = () => {
    axios({
      method: "get",
      url: process.env.REACT_APP_BASEURL + "/api/service_provider/list",
      headers: { Authorization: state.userToken },
    })
      .then((response) => {
        console.log(response.data);

        // Filter sellers based on selected services
        const filteredSellers = response.data.data.filter((seller) =>
          doesSellerHaveSelectedServices(seller)
        );

        setMainState((prev) => ({
          ...prev,
          resAllSellers: filteredSellers,
        }));
      })
      .catch((error) => {
        // defaultAPIErrorHandler(error)
  ...
  console.log(mainState.resAllSellers);
  ...
  const handleInputChange2 = (e, serviceId) => {
  ...
  //   }));
  // }, [location.pathname]);
  const approvedSellers = mainState.resAllSellers.filter(
      ...
      >
        <div className="row">
          ...
          <section className="col-lg-9">
              ...
              <div className="col-md-6 d-flex  align-items-center">
                <h1 className="ml-3">
                  <label id="price-filter-count">
                    {" "}
                    {approvedSellers.length} Items found{" "}
                  </label>
                </h1>
              </div>
                      ...
                      >
                        ...
                        >
                          <div
                            ...
                            style={{ cursor: "pointer" }}
                          >
                            <Link to={`/serviceView/${item._id}`}>
                              <img
                                src={item.company_logo || dummyProductImage}
            ...
            {/* <div className="container mb-md-4" style={{marginTop : "1rem"}}>
                <div className="row">
